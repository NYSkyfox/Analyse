...
> ✅ **实证核对（2026-09-12 抓包）**：本报告 2.1 节关于 `/*//` 是「载荷前缀」的结论**正确**，已由 captured_logs 抓包证实。详见《10_行为管控报文核查定稿.md》。
>
...
if (FID_conflict_size(param_1 + 0x548) < 0x8000) {   // 载荷长度检查
    ...
    FUN_10008b40(&auStack_58, "/*//");               // ⚡ 载荷前缀（抓包实证）
    FUN_1009e070(*(void **)(param_1+0x30), pvVar3, sVar2);  // 遍历发送
}
```

> ⚡ **实证（2026-09-12 抓包）**：type=500 的载荷**带** `/*//` 前缀。真实抓包报文 106 字节 = 16B 头 + 90B 载荷，载荷前 4 字节 `2f2a2f2f` 即 `/*//`。发送缓冲 = 16B 头 + `/*//` + JSON。
...
- 但已知：载荷带 `"/*//"` 前缀（抓包实证），长度需 < 0x8000，且与 Teacher.exe 的 CtrlCode JSON（`0x01网络/0x02键盘/0x10应用/0x100设备`）**很可能就是同一内容的两种编码**
...
1. `this+0x548` 载荷（type=500）的**确切字段结构**（`/*//` + JSON）
