...
> ⚠️ **修正声明（2026-09-12 定稿）**：本报告 2.1 节关于 `/*//` 是「载荷前缀」的结论**错误**。经逐指令反汇编，`/*//` 为死代码；所谓“抓包实证”系循环论证（测试包照抄了错误报告）。详见《10_行为管控报文核查定稿.md》。
>
...
> ⚡ **定稿（2026-09-12 反汇编）**：type=500 的载荷**不带** `/*//` 前缀。发送缓冲 = 16B 头 + 纯 JSON。（“抓包带前缀”系测试包照抄错误报告所致，非原生格式。）
...
- 但已知：载荷**无前缀**（`/*//` 为死代码），长度需 < 0x8000，且与 Teacher.exe 的 CtrlCode JSON（`0x01网络/0x02键盘/0x10应用/0x100设备`）**很可能就是同一内容的两种编码**
...
1. 组装 `[500][0][0][len] + payload`（payload 用 CtrlCode JSON；如需保险可现场双格式对照测试）
