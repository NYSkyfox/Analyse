...
if (FID_conflict_size(param_1 + 0x548) < 0x8000) {   // 载荷长度检查
    ...
    FUN_10008b40(&auStack_58, "/*//");               // ⚡ 载荷带 /*// 前缀
    FUN_1009e070(*(void **)(param_1+0x30), pvVar3, sVar2);  // 遍历发送
}
```

> ⚡ **新发现**：type=500 的载荷用 `"/*//"` 作为前缀（在 MainLogic.dll 出现 32 次、Teacher.exe 17 次、Student.exe 2 次），且多个日志字符串（StartUnicast/SendMsgToTerminal）周围都带它。**这是消息格式标记，不是 JSON 魔数** —— 纠偏了 Ghidra 之前怀疑 `udPPPPj` 是线格式魔数的方向。
...
- 但已知：载荷带 `"/*//"` 前缀，长度需 < 0x8000，且与 Teacher.exe 的 CtrlCode JSON（`0x01网络/0x02键盘/0x10应用/0x100设备`）**很可能就是同一内容的两种编码**
...
| 载荷"魔法前缀" | udPPPPj(疑) | ✅ **`/*//` 前缀**（MainLogic 32 次，需进一步定字段） |
...
1. `this+0x548` 载荷（type=500）的**确切字段结构**（`/*//` 前缀后面的 JSON 长什么样）
